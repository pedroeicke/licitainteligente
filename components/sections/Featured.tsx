        ...
        <div className="mt-14 grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-3">
          ...
          <Reveal className="md:col-span-2 lg:col-span-2 lg:row-span-2">
            <Link
              ...
              className="group relative flex h-full min-h-[440px] flex-col overflow-hidden rounded-[28px] bg-brand-500 p-8 text-white md:p-10"
            >
              <div
                aria-hidden
                className="absolute -bottom-32 -right-32 h-96 w-96 rounded-full bg-brand-400 opacity-70 blur-3xl transition-transform duration-700 group-hover:scale-125"
              />
              ...
              <div className="relative mt-auto pt-16">
                <span className="grid h-14 w-14 place-items-center rounded-2xl bg-white text-brand-600">
                ...
                <span className="mt-8 inline-flex items-center gap-2 rounded-full bg-white px-5 py-3 text-sm font-semibold text-ink transition-transform duration-300 group-hover:translate-x-1">
          ...
          {rest.map((c, i) => (
            <Reveal key={c.slug} delay={0.08 * (i + 1)} className={clsx(i === 2 && "md:col-span-2 lg:col-span-3")}>
              <Link
                href={`/cursos/${c.slug}`}
                className="group flex h-full flex-col rounded-[28px] border border-line bg-white p-7 transition-all duration-500 hover:-translate-y-1 hover:border-brand-200 hover:shadow-lift"
              >
                <div className="flex items-start justify-between gap-4">
                  <span className="grid h-12 w-12 place-items-center rounded-2xl bg-ink text-white transition-colors duration-500 group-hover:bg-brand-500">
